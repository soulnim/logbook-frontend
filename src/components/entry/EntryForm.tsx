...
// Exclude COMMIT from manual creation — it's auto-generated by GitHub
const MANUAL_TYPES = Object.entries(ENTRY_TYPE_META).filter(([type]) => type !== 'COMMIT') as [EntryType, typeof ENTRY_TYPE_META[EntryType]][]
...
export function EntryForm({ date, entry, onClose }: EntryFormProps) {
  ...
  // ── COMMIT edit form — special layout ─────────────────────────────────────
  if (isCommit && isEditing) {
    return (
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 animate-fade-in">
        ...
        {/* Notes (editable — this is their reflection) */}
        <Textarea
          label="Your notes"
          value={content}
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4 animate-fade-in">
      {/* Type selector (no COMMIT option for manual creation) */}
